...
def range_for_material_objtype(
    ...
    *,
    ...
    if (
        ...
        if (type_cte := ctx.type_ctes.get(key)) is None:
            with ctx.newrel() as sctx:
                ...
                if not ctx.env.expand_inhviews:
    ...
    # parts of the query are being referred to.
        ...
        # XXX: This is a workaround for #4493
        and not typeref.name_hint.module == 'schema'
    ):
        ops = []
        for subref in [typeref, *irtyputils.get_typeref_descendants(typeref)]:
            rvar = range_for_material_objtype(
                ...
                ignore_rewrites=ignore_rewrites,  # XXX?
                ctx=ctx,
            )
            ...
            # pathctx.put_path_bond(qry, sub_path_id)
...
def range_for_ptrref(
    ...
    # XXX: I worry about the overlay interaction
    # EXPLAIN thing, see range_for_material_objtype for details.
    lrefs: List[irast.BasePointerRef]
    if (
        ctx.env.expand_inhviews
        and include_descendants
        # XXX?
        # not ptrref.union_is_concrete
        and not for_mutation

        # XXX: This is a workaround for #4493
        and not ptrref.out_source.name_hint.module == 'schema'  # type: ignore
